...
def main(argv):
    ...
    for fname in args:
        ...
        fp.close()
        parseTrans(outfp.getvalue())
    device.close()


def parseTrans(transContent, outfp = None):
...
def findCurQuarter(transContent, curAQ, yearPattern):    
    ...
    log("Searching for current quarter in transcript")
    ...
    if m == None and curAQ.year == "No Year":
        ...
        return -1
    curYear = m.group()
    transContent = transContent[m.start() + len(curYear):]
    ...
    log ("Found quarter: " + curAQ.toString())
    return m.start() + len(curQuarter)    
...
def autoResolveAmbiguity(course, transContent, match):
	...
	for numAlphaChars in range(0,3):
		...
		for courseObj in courseList:
			...
			if curCourseId == courseIdCand and transcriptMatches(baseCourseId, numAlphaChars, courseObj['course_title'], transContent, match, numDeletedChars):
				course.id = courseIdCand
				log ("Selected course: " + curCourseId + " : " + courseObj['course_title'], True)
...
def getCourseList(courseId):
	courseList = []
	json_data=open('../Data/courses_json_2013')
